...
class RegistrationController extends Controller
{
    ...
    public function show(Registration $pendaftaran)
    {
        ...
        if ($pendaftaran->status === 'diterima' && $pendaftaran->ticket_code) {
            // Generate PNG QR code (4x scale) encoded as base64 for display
    ...
    public function scan()
    {
        ...
        // Fetch recent verification logs to display on the page
    ...
    public function verifyScan(Request $request)
    {
        ...
        // Common data to return for UI feedback
        $responseData = [
            ...
            'status' => $registration->status, // Include status for UI display
        ];
        ...
        if ($registration->status === 'ditolak') {
            return response()->json([
                ...
                'data' => $responseData
            ], 200); // Return 200 so axios/fetch can handle the data object easily
        }

        // 2. Check if already verified (checked-in)
        if ($registration->verified_at !== null) {
            return response()->json([
                'status' => 'warning',
                'message' => 'Peserta sudah diverifikasi sebelumnya pada ' . $registration->verified_at->format('d M Y H:i') . ( $registration->verified_by ? ' oleh ' . $registration->verified_by : ''),
                'data' => $responseData
            ]);
        }

        // 3. Perform Verification (Check-in)
        $updateData = [
            'verified_at' => now(),
        ...
        if ($registration->status === 'pending') {
            ...
            $responseData['status'] = 'diterima'; // Update for response
        }
    ...
    {
        // Authorization - only user who registered or admin can download
        if (Auth::user()->role !== 'admin' && Auth::id() !== $pendaftaran->user_id) {
            abort(403);
        }
        ...
        // Generate PNG at 4px per QR module
        $pngData = $this->generateQrPng($pendaftaran->ticket_code, 4);
    ...
    public function downloadCertificate(Registration $pendaftaran)
    {
        ...
        // Generate QR PNG (4x scale) as base64 to embed in HTML
        $qrPngBase64 = base64_encode($this->generateQrPng($pendaftaran->ticket_code, 4));

        // Generate HTML document with embedded QR
    ...
    /**
     * Generate a PNG image of the QR code using GD at the given pixel-per-module scale.
     * Scale 4 means each QR module is 4x4 pixels (4x scale).
     *
     * @param  string  $content  The data to encode in the QR code
     * @param  int     $scale    Pixels per QR module (default 4 = 4x scale)
     * @return string            Raw PNG binary data
     */
    private function generateQrPng(string $content, int $scale = 4): string
    {
        // Encode content into QR code using BaconQrCode encoder
        $qrCode = Encoder::encode($content, ErrorCorrectionLevel::M());
        $matrix  = $qrCode->getMatrix();
        $matrixSize = $matrix->getWidth();

        // Quiet zone (margin) of 4 modules as per QR spec
        $margin = 4;
    ...
    private function generateCertificateHtml(Registration $pendaftaran, string $qrPngBase64 = '')
    {
        ...
        // Build QR image tag – embedded as base64 PNG if available, otherwise blank placeholder
        ...
        <body>
            <div class="container">
                ...
                <div class="section">
                    ...
                    <div class="row">
                        ...
                        <div class="field">
                            ...
                            <div class="field-value">{$year}</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="field">
                            <label class="field-label">Usia</label>
                            <div class="field-value">{$age} tahun</div>
                        </div>
                        <div class="field">
                ...
                <div class="section">
                    ...
                    <div class="row full">
                        <div class="field">
                            ...
                            <div class="field-value">{$eventTitle}</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="field">
                            <label class="field-label">Tanggal Event</label>
                ...
                <div class="section">
                    ...
                    <div class="row">
                        <div class="field">
                            <label class="field-label">Kode Tiket</label>
                            <div class="field-value">{$ticketCode}</div>
                        </div>
                        <div class="field">
                            <label class="field-label">Tanggal Pendaftaran</label>
